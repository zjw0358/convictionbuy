...
class CbTasks:
    ...
    '''
    '''
    filter1=-g st_sma&ma10b<20&ma10b<ma10s&px>ma200&px>ma50,st_dmi&dmi_buy<20,ms_zack&erd -i 1,2,3 --feed yahoo -s 2013-01-01
    '''
    def readMarketscanFilter(self,sect):
        idx = 1
        arglst = []
        while (True):
            try:
                filterName = "filter"+str(idx)        
                #print "section",sect, filterName   
                filterStr = self.msconfig.getConfig(sect,filterName)
                if (filterStr==""):
                    break;
                arglst.append(filterStr)
                idx+=1
            except:
                #print "break"
                break
    ...
    def processMarketscan(self,sect):
        print "====",sect,"task"  
        try:
            taskstr = self.msconfig.getConfig(sect,'task')
        except:
            taskstr = ""
            pass
        
        if (taskstr=="load_data"):
            paramstr = self.msconfig.getConfig(sect,'param')
            self.scaner.loadDataTask(paramstr)
            pass
        else:
            arglst = self.readMarketscanFilter(sect)
            df = self.scaner.scanTask(arglst)            
            
            self.dfdict[sect] = df    
            print('Finished scan task')   
